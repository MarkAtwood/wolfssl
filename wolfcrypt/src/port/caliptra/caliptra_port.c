 ...
 * ========================================================================= */
    ...
    /* Stack-buffers path: the buffer is on the caller's stack frame and will
     * be released when the function returns.  Zero it now anyway so the stack
     * slot does not retain sensitive material across the return / future
     * stack-frame reuse. */
    #define CALIPTRA_FREE_SENSITIVE(ptr) \
        do { if ((ptr) != NULL) wc_ForceZero((ptr), sizeof(*(ptr))); } while(0)
    ...
    /* Heap-buffers path: zero the allocation before returning it to the
     * allocator so the freed memory does not retain sensitive material.
     * sizeof(*(ptr)) is well-defined because ptr is still typed at the
     * call site (CmFooReq* / CmFooResp*). */
    #define CALIPTRA_FREE_SENSITIVE(ptr) \
        do { if ((ptr) != NULL) { \
            wc_ForceZero((ptr), sizeof(*(ptr))); \
            XFREE((ptr), NULL, DYNAMIC_TYPE_TMP_BUFFER); (ptr) = NULL; \
        } } while(0)
...
hmac_done:
    /* req carries the input message; resp carries the MAC.  Both can
     * carry sensitive material for HMAC-based KDF or password use. */
    CALIPTRA_FREE_SENSITIVE(req);
    CALIPTRA_FREE_SENSITIVE(resp);
    if (ret != 0)
...
enc_done:
    ...
    CALIPTRA_FREE_SENSITIVE(init_req);
    CALIPTRA_FREE_SENSITIVE(init_resp);
    CALIPTRA_FREE_SENSITIVE(upd_req);
    CALIPTRA_FREE_SENSITIVE(upd_resp);
    CALIPTRA_FREE_SENSITIVE(final_req);
    CALIPTRA_FREE_SENSITIVE(final_resp);
    return ret;
}
...
static int caliptra_aesgcm_decrypt(wc_CryptoInfo* info)
{
    ...
    if (sz > 0) {
        ...
        /* Copy plaintext to output */
        {
            ...
            /* Validate firmware-reported size before using it: defensive
             * upper bound matching upd_resp->plaintext[CMB_MAX_DATA_SIZE].
             * Mirrors the encrypt path's check on ciphertext_size at the
             * sibling site above. */
            if (pt_sz > (word32)CMB_MAX_DATA_SIZE) {
                ret = WC_HW_E; goto dec_done;
            }
            if (pt_sz > 0 && out != NULL) {
...
dec_done:
     ...
     * ciphertext less so but zero everything for defense-in-depth. */
    CALIPTRA_FREE_SENSITIVE(init_req);
    CALIPTRA_FREE_SENSITIVE(init_resp);
    CALIPTRA_FREE_SENSITIVE(upd_req);
    CALIPTRA_FREE_SENSITIVE(upd_resp);
...
sign_done:
    /* req: message digest (intermediate signing material);
     * resp: signature components.  Zero both. */
    CALIPTRA_FREE_SENSITIVE(req);
    CALIPTRA_FREE_SENSITIVE(resp);
    return ret;
}
...
import_done:
    /* req: raw key bytes (the most sensitive material in this port).
     * resp: CMK handle (an attacker-useful primitive).  Zero both. */
    CALIPTRA_FREE_SENSITIVE(req);
    CALIPTRA_FREE_SENSITIVE(resp);
    return ret;
}
