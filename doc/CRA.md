...
## SBOM Coverage for Other wolfSSL Products

If your product uses other wolfSSL-family libraries alongside wolfssl, each has
its own `make sbom` target using the same `scripts/gen-sbom` script:

```sh
# In each product's build directory, after building:
make sbom WOLFSSL_DIR=/path/to/wolfssl

# For wolfBoot (bootloader — specify your build configuration):
make sbom TARGET=stm32h7 SIGN=ECC256 HASH=SHA256

# For wolfHSM (no autotools — invoke gen-sbom directly):
python3 $WOLFSSL_DIR/scripts/gen-sbom \
    --name wolfhsm --version 1.4.0 \
    --supplier "wolfSSL Inc." \
    --srcs $WOLFHSM_DIR/src/*.c \
    --options-h $WOLFSSL_DIR/include/wolfssl/options.h
```

Products with `make sbom` support: wolfSSH, wolfTPM, wolfMQTT, wolfsentry,
wolfBoot, wolfHSM, wolfEngine, wolfProvider, wolfSCEP, wolfCLU.

See each product's README for product-specific invocation details, and
`doc/SBOM.md` for the full product cross-reference table.
