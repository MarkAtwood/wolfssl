 ...
 * Decrypt: caller provides IV as three little-endian u32 words in
 * DecryptInitReq.  See the per-struct comments below (and caliptra_port.c
 * line ~924) for the byte-order details: raw memcpy of 12 LE IV bytes into
 * a word32[3] field produces the correct LE u32 values on LE hosts, which
 * is what the firmware reads.
